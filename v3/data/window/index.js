...
chrome.storage.local.get({
  video: true,
  audio: true
}, prefs => navigator.mediaDevices.getUserMedia(prefs).then(e => {
  ...
  notify.textContent = e.message + '\n\nVisit chrome://settings/content?search=permissions in a browser tab to allow permission';
}));
  ...
  const capture = {
    ...
    download() {
      if (capture.progress === 0 && recorder.state === 'inactive') {
        const end = new Date();
        const filename = chrome.runtime.getManifest().name + ' ' +
...
audio.addEventListener('change', e => chrome.storage.local.set({
